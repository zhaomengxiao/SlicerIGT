...
void vtkSlicerVolumeResliceDriverLogic ::PrintSelf(ostream &os,
                                                   vtkIndent indent) {
  this->Superclass::PrintSelf(os, indent);

  os << indent << "Number of observed nodes: " << this->ObservedNodes.size()
     << std::endl;
...
void vtkSlicerVolumeResliceDriverLogic ::SetDriverForSlice(
    std::string nodeID, vtkMRMLSliceNode *sliceNode) {
  ...
  vtkMRMLTransformableNode *tnode =
...
void vtkSlicerVolumeResliceDriverLogic ::SetModeForSlice(
    int mode, vtkMRMLSliceNode *sliceNode) {
  if (sliceNode == NULL) {
    return;
  }
  ...
  modeSS << mode;
  sliceNode->SetAttribute(VOLUMERESLICEDRIVER_MODE_ATTRIBUTE,
                          modeSS.str().c_str());

  this->UpdateSliceIfObserved(sliceNode);
}

void vtkSlicerVolumeResliceDriverLogic ::SetRotationForSlice(
    double rotation, vtkMRMLSliceNode *sliceNode) {
  if (sliceNode == NULL) {
    return;
  }
  ...
  rotationSs << rotation;
  sliceNode->SetAttribute(VOLUMERESLICEDRIVER_ROTATION_ATTRIBUTE,
                          rotationSs.str().c_str());

  this->UpdateSliceIfObserved(sliceNode);
}

void vtkSlicerVolumeResliceDriverLogic ::SetFlipForSlice(
    bool flip, vtkMRMLSliceNode *sliceNode) {
  if (sliceNode == NULL) {
    return;
  }
  ...
  flipSs << flip;
  sliceNode->SetAttribute(VOLUMERESLICEDRIVER_FLIP_ATTRIBUTE,
                          flipSs.str().c_str());

  this->UpdateSliceIfObserved(sliceNode);
}

void vtkSlicerVolumeResliceDriverLogic ::AddObservedNode(
  ...
  // Collect event IDs that we need to observe to detect changes in the driving
  // node
  vtkNew<vtkIntArray> modifiedEventsToObserve;

  // Default events
  modifiedEventsToObserve->InsertNextValue(vtkCommand::ModifiedEvent);
  modifiedEventsToObserve->InsertNextValue(
  ...
  if (customContentModifiedEvents) {
    for (vtkIdType i = 0; i < customContentModifiedEvents->GetNumberOfValues();
         i++) {
...
void vtkSlicerVolumeResliceDriverLogic ::ClearObservedNodes() {
  for (unsigned int i = 0; i < this->ObservedNodes.size(); ++i) {
...
void vtkSlicerVolumeResliceDriverLogic::SetMRMLSceneInternal(
...
/// This method is called by Slicer after each significant MRML scene event
/// (import, load, etc.)
void vtkSlicerVolumeResliceDriverLogic ::UpdateFromMRMLScene() {
  ...
  // Check if any of the slice nodes contain driver transforms that need to be
  // observed.

  vtkCollection *sliceNodes =
  ...
  for (sliceIt->InitTraversal(); !sliceIt->IsDoneWithTraversal();
       sliceIt->GoToNextItem()) {
    vtkMRMLSliceNode *slice =
        vtkMRMLSliceNode::SafeDownCast(sliceIt->GetCurrentObject());
    if (slice == NULL) {
      continue;
    }
    const char *driverCC =
        slice->GetAttribute(VOLUMERESLICEDRIVER_DRIVER_ATTRIBUTE);
    if (driverCC == NULL) {
      continue;
    }
    ...
    vtkMRMLTransformableNode *driverTransformable =
        vtkMRMLTransformableNode::SafeDownCast(driverNode);
...
//---------------------------------------------------------------------------
void vtkSlicerVolumeResliceDriverLogic ::OnMRMLSceneNodeAdded(
    vtkMRMLNode *vtkNotUsed(node)) {}

//---------------------------------------------------------------------------
void vtkSlicerVolumeResliceDriverLogic ::OnMRMLSceneNodeRemoved(
    vtkMRMLNode *vtkNotUsed(node)) {}

void vtkSlicerVolumeResliceDriverLogic ::OnMRMLNodeModified(
    vtkMRMLNode *vtkNotUsed(node)) {
  std::cout << "Observed node modified." << std::endl;
}

void vtkSlicerVolumeResliceDriverLogic ::ProcessMRMLNodesEvents(
  ...
  vtkMRMLTransformableNode *callerNode =
  ...
  for (int i = 0; i < sliceNodes->GetNumberOfItems(); ++i) {
    ...
    sliceIt->GoToNextItem();
    const char *driverCC =
        sliceNode->GetAttribute(VOLUMERESLICEDRIVER_DRIVER_ATTRIBUTE);
    if (sliceNode != NULL && driverCC != NULL &&
...
void vtkSlicerVolumeResliceDriverLogic ::UpdateSliceByTransformableNode(
    vtkMRMLTransformableNode *tnode, vtkMRMLSliceNode *sliceNode) {
  vtkMRMLLinearTransformNode *transformNode =
      vtkMRMLLinearTransformNode::SafeDownCast(tnode);
  if (transformNode != NULL) {
    this->UpdateSliceByTransformNode(transformNode, sliceNode);
  }

  vtkMRMLScalarVolumeNode *imageNode =
      vtkMRMLScalarVolumeNode::SafeDownCast(tnode);
  if (imageNode != NULL) {
  ...
  vtkMRMLAnnotationRulerNode *rulerNode =
      vtkMRMLAnnotationRulerNode::SafeDownCast(tnode);
  if (rulerNode != NULL) {
    this->UpdateSliceByRulerNode(rulerNode, sliceNode);
  }
}

void vtkSlicerVolumeResliceDriverLogic ::UpdateSliceByTransformNode(
    vtkMRMLLinearTransformNode *tnode, vtkMRMLSliceNode *sliceNode) {
  if (!tnode) {
    return;
  }
...
void vtkSlicerVolumeResliceDriverLogic ::UpdateSliceByImageNode(
    vtkMRMLScalarVolumeNode *inode, vtkMRMLSliceNode *sliceNode) {
  ...
  vtkSmartPointer<vtkMatrix4x4> rtimgTransform =
      vtkSmartPointer<vtkMatrix4x4>::New();
  ...
  // imageData may be NULL if volume reslice driver is active while loading an
  // image. Slice position and orientation is stored in the node, so we can
  // still update the slice pose.
  if (imageData != NULL) {
  ...
  vtkMRMLLinearTransformNode *parentNode =
      vtkMRMLLinearTransformNode::SafeDownCast(
          volumeNode->GetParentTransformNode());
  if (parentNode) {
...
void vtkSlicerVolumeResliceDriverLogic ::UpdateSliceByMarkupsNode(
    vtkMRMLMarkupsNode *markupsNode, vtkMRMLSliceNode *sliceNode) {
  vtkMRMLMarkupsPlaneNode *planeNode =
      vtkMRMLMarkupsPlaneNode::SafeDownCast(markupsNode);
  if (planeNode) {
  ...
  vtkMRMLMarkupsLineNode *lineNode =
      vtkMRMLMarkupsLineNode::SafeDownCast(markupsNode);
  if (lineNode) {
    double position1[3];
    double position2[3];
...
void vtkSlicerVolumeResliceDriverLogic ::UpdateSliceByRulerNode(
    vtkMRMLAnnotationRulerNode *rnode, vtkMRMLSliceNode *sliceNode) {
  double position1[4];
  double position2[4];
...
void vtkSlicerVolumeResliceDriverLogic ::UpdateSliceByPoint(
    double posistion1[3], vtkMRMLSliceNode *sliceNode) {
...
void vtkSlicerVolumeResliceDriverLogic ::UpdateSliceByLine(
    double position1[3], double position2[3], vtkMRMLSliceNode *sliceNode) {
  double t[3];
  double s[3];
  double n[3];
  double nlen;
 ...
 */
void vtkSlicerVolumeResliceDriverLogic ::UpdateSlice(
    vtkMatrix4x4 *driverToRASMatrix, vtkMRMLSliceNode *sliceNode) {
  // Default values determining the default SliceToDriver transform.
  int mode = MODE_NONE;
  int rotation = 0;
  int flip = 0;

  // Default values for SliceToDriver can be modified by driver node attributes.
  // Read them.

  const char *modeCC =
      sliceNode->GetAttribute(VOLUMERESLICEDRIVER_MODE_ATTRIBUTE);
  if (modeCC != NULL) {
    std::stringstream modeSS(modeCC);
    modeSS >> mode;
  }

  const char *rotationCC =
      sliceNode->GetAttribute(VOLUMERESLICEDRIVER_ROTATION_ATTRIBUTE);
  if (rotationCC != NULL) {
  ...
  const char *flipCC =
      sliceNode->GetAttribute(VOLUMERESLICEDRIVER_FLIP_ATTRIBUTE);
  if (flipCC != NULL) {
  ...
  double sliceX[3] = {driverToRASMatrix->GetElement(0, 0),
                      driverToRASMatrix->GetElement(1, 0),
                      driverToRASMatrix->GetElement(2, 0)};
  double sliceY[3] = {driverToRASMatrix->GetElement(0, 1),
                      driverToRASMatrix->GetElement(1, 1),
                      driverToRASMatrix->GetElement(2, 1)};
  double sliceZ[3] = {driverToRASMatrix->GetElement(0, 2),
                      driverToRASMatrix->GetElement(1, 2),
                      driverToRASMatrix->GetElement(2, 2)};
  ...
  if (fabs(vtkMath::Dot(sliceX, sliceY)) < tolerance &&
    ...
    // Vectors are orthogonal, we just have to make sure they are normalized as
    // well
    for (int i = 0; i < 3; i++) {
      // Normalize i-th column vector
      double driverColumn[3] = {driverToRASMatrix->GetElement(0, i),
                                driverToRASMatrix->GetElement(1, i),
                                driverToRASMatrix->GetElement(2, i)};
      ...
      driverToRASMatrixOrthoNormalized->SetElement(
          i, 3, driverToRASMatrix->GetElement(i, 3));
    }
  } else {
    vtkWarningMacro("Volume reslice driver matrix is not orthonormal. Matrix "
                    "will be orthonormalized before set in SliceToRAS.");
    double in[3][3];
    double out[3][3];
    for (int i = 0; i < 3; i++) {
  ...
  vtkSmartPointer<vtkTransform> driverToRASTransform =
      vtkSmartPointer<vtkTransform>::New();
  driverToRASTransform->SetMatrix(
      driverToRASMatrixOrthoNormalized.GetPointer());
  ...
  vtkSmartPointer<vtkTransform> driverToRasTranslation =
      vtkSmartPointer<vtkTransform>::New();
  ...
  vtkSmartPointer<vtkTransform> driverToRasRotation =
      vtkSmartPointer<vtkTransform>::New();
  driverToRasRotation->Identity();
  driverToRasRotation->RotateWXYZ(
      driverToRasOrientationVector[0], driverToRasOrientationVector[1],
      driverToRasOrientationVector[2], driverToRasOrientationVector[3]);
  driverToRasRotation->Update();

  vtkSmartPointer<vtkTransform> sliceToDriverTransform =
      vtkSmartPointer<vtkTransform>::New();
  sliceToDriverTransform->Identity();

  vtkSmartPointer<vtkTransform> sliceToRASTransform =
      vtkSmartPointer<vtkTransform>::New();
  ...
  case MODE_ImplantCurveTangent: {
    vtkNew<vtkMatrix4x4> sliceToRASMatrix;
    double zAxis[3] = {driverToRASMatrixOrthoNormalized->GetElement(0, 0),
                       driverToRASMatrixOrthoNormalized->GetElement(1, 0),
                       driverToRASMatrixOrthoNormalized->GetElement(2, 0)};
    double refAxis[3] = {0.0, 0.0, 1.0};
    double xAxis[3] = {0.0, 0.0, 0.0};
    double yAxis[3] = {0.0, 0.0, 0.0};

    vtkMath::Cross(refAxis, zAxis, xAxis);
    if (vtkMath::Norm(xAxis) < 1e-6) {
      // If zAxis is parallel to (0,0,1), use (0,1,0) as reference
      refAxis[0] = 0.0;
      refAxis[1] = 1.0;
      refAxis[2] = 0.0;
      vtkMath::Cross(refAxis, zAxis, xAxis);
    ...
    sliceToRASMatrix->SetElement(0, 0, xAxis[0]);
    sliceToRASMatrix->SetElement(1, 0, xAxis[1]);
    sliceToRASMatrix->SetElement(2, 0, xAxis[2]);
    sliceToRASMatrix->SetElement(0, 1, yAxis[0]);
    sliceToRASMatrix->SetElement(1, 1, yAxis[1]);
    sliceToRASMatrix->SetElement(2, 1, yAxis[2]);
    sliceToRASMatrix->SetElement(0, 2, zAxis[0]);
    sliceToRASMatrix->SetElement(1, 2, zAxis[1]);
    sliceToRASMatrix->SetElement(2, 2, zAxis[2]);
    sliceToRASMatrix->SetElement(0, 3, driverToRasTranslationVector[0]);
    sliceToRASMatrix->SetElement(1, 3, driverToRasTranslationVector[1]);
    sliceToRASMatrix->SetElement(2, 3, driverToRasTranslationVector[2]);

    sliceToRASTransform->SetMatrix(sliceToRASMatrix);
    sliceToRASTransform->Update();
  } break;
  case MODE_ImplantCurveNormal: {
    vtkNew<vtkMatrix4x4> sliceToRASMatrix;
    double zAxis[3] = {driverToRASMatrixOrthoNormalized->GetElement(0, 1),
                       driverToRASMatrixOrthoNormalized->GetElement(1, 1),
                       driverToRASMatrixOrthoNormalized->GetElement(2, 1)};
    double refAxis[3] = {0.0, 0.0, 1.0};
    double xAxis[3] = {0.0, 0.0, 0.0};
    double yAxis[3] = {0.0, 0.0, 0.0};
...
void vtkSlicerVolumeResliceDriverLogic ::UpdateSliceIfObserved(
    vtkMRMLSliceNode *sliceNode) {
  if (sliceNode == NULL) {
    return;
  }
